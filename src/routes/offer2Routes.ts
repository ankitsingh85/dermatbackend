...
import Offer2 from "../models/offer2";
...
router.get("/", async (req: Request, res: Response) => {
  try {
    const offers = await Offer2.find().sort({ createdAt: -1 });
...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    const newOffer = new Offer2({ imageBase64 });
    await newOffer.save();
...
router.put("/:id", async (req: Request, res: Response) => {
  try {
    ...
    const updated = await Offer2.findByIdAndUpdate(
      req.params.id,
      { imageBase64 },
      { new: true }
    );
